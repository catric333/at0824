...
public class RentalAgreement {
  ...
  public RentalAgreement(final Tool tool) {
    ...
    this.preDiscountCharge = this.dailyRentalCharge.multiply(
        BigDecimal.valueOf(this.numChargeableDays)).setScale(2, RoundingMode.HALF_UP);
    this.discountAmount = this.preDiscountCharge.multiply(
            BigDecimal.valueOf((this.tool.getDiscountPercentage()) / 100.0))
        .setScale(2, RoundingMode.HALF_UP);
    this.finalChargeAmount = this.preDiscountCharge.subtract(this.discountAmount)
        .setScale(2, RoundingMode.HALF_UP);
  }
  ...
  private boolean isHoliday(Calendar c) {
    ...
    if (!isWeekend(c)) {
      if ((c.get(Calendar.DAY_OF_WEEK) == Calendar.MONDAY) && (c.get(Calendar.DAY_OF_MONTH) <= 7)
        ...
        return true;
          ...
          (c.get(Calendar.MONTH) == Calendar.JULY)
              ...
              || (
              ((c.get(Calendar.DAY_OF_MONTH) == 5) && (c.get(Calendar.DAY_OF_WEEK)
                  == Calendar.MONDAY))
                  || ((c.get(Calendar.DAY_OF_MONTH) == 3) && (c.get(Calendar.DAY_OF_WEEK)
                  == Calendar.FRIDAY))
          )
          )) {
        return true;
